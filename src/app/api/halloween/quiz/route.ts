...
import { afterQuizCorrect, currentQuizStep } from "@/lib/halloween/progress";
import { answersMatch } from "@/lib/halloween/quiz-normalize";
import {
  ensureDevice,
...
import { halloweenQuizBodySchema } from "@/lib/validation";

export async function POST(request: Request) {
  const json = await request.json().catch(() => null);
  const parsed = halloweenQuizBodySchema.safeParse(json);
  if (!parsed.success) {
